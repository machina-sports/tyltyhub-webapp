...
const getImageUrl = (article: any): string => {
  ...
  if (article.value?.["event-details"]?.match) {
    ...
    return `https://placehold.co/1200x600/2A9D8F/FFFFFF?text=${encodeURIComponent(match)}`;
  }
  
  // Fallback to using title if available
  return `https://placehold.co/1200x600/2A9D8F/FFFFFF?text=${encodeURIComponent(article.value?.title || 'Article')}`;
};
